...
echo -e "${BOLD}📄 Commit Messages:${RESET}\n$COMMITS"

# ✅ Correct JSON with role:user
jq -n --arg text "$PROMPT" \
  '{contents: [{role: "user", parts: [{text: $text}]}]}' > "$TMP_JSON"
...
while [ $ATTEMPT -le $MAX_RETRIES ]; do
  ...
  CURL_PID=$!
  spinner "$CURL_PID" "Generating Daily Report... (Attempt $ATTEMPT/$MAX_RETRIES)"
  wait "$CURL_PID"
  ...
  echo -e "${YELLOW}⚠️ Attempt $ATTEMPT failed. Retrying in $RETRY_DELAY seconds...${RESET}"
  ATTEMPT=$((ATTEMPT+1))
  sleep $RETRY_DELAY
done

if [ "$SUCCESS" = false ]; then
  echo -e "${RED}❌ Failed to get report from API after $MAX_RETRIES attempts.${RESET}"
  echo "Last response:"
  cat "$TMP_RESPONSE"
  exit 1
fi
...
while IFS= read -r line; do
  for ((i=0; i<${#line}; i++)); do
    printf "%s" "${line:$i:1}"
    sleep 0.015  # typing speed per char
  done
  printf "\n"
