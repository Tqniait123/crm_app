...
import 'package:project_name/config/routes/app_router.dart';
import 'package:project_name/core/extensions/context_extensions.dart';
import 'package:project_name/core/extensions/dimensions_manager.dart';
import 'package:project_name/core/extensions/num_extension.dart';
import 'package:project_name/core/extensions/page_transition.dart';
import 'package:project_name/core/extensions/string_extensions.dart';
import 'package:project_name/core/extensions/text_style_extension.dart';
import 'package:project_name/core/extensions/theme_extension.dart';
import 'package:project_name/core/utils/widgets/scaffold/blurred_circle.dart';
import 'package:shimmer/shimmer.dart';

Duration transitionDuration = const Duration(milliseconds: 300);

extension WidgetExtension on Widget {
  Widget withBlurredCircle() => Stack(children: [const BlurredBackgroundCircle(), this]);
  ...
  Widget setCircleBorder(BuildContext context, {double? width, Color? color, double padding = 0}) {
    return Container(
      padding: padding.edgeInsetsAll.w,
      decoration: BoxDecoration(
  ...
  ClipRRect clipRRectOnly({double bottomLeft = 0, double bottomRight = 0, double topLeft = 0, double topRight = 0}) {
    return ClipRRect(
...
extension LayoutExtensions on Widget {
  ...
  Widget toBottomNavBar({bool enableKeyboardPadding = false}) {
    final context = rootNavigatorKey.currentContext!;
    return withSafeArea(
      minimum: AppSize.screenPadding.edgeInsetsHorizontal.copyWith(
        bottom: enableKeyboardPadding ? context.keyboardPadding + 16 : 16,
      ),
    ).paddingTop(8).setContainerToView(color: context.scaffoldBackgroundColor);
  }
}
...
extension AnimationExtension on Widget {
  ...
  Widget withOpacity({required double opacity, int durationInSecond = 1, Duration? duration}) {
...
extension TextEx on Text {
      ...
      builder: (context, value, child) {
        return InkWell(
          ...
          child: AnimatedDefaultTextStyle(
            ...
            style:
                style?.copyWith(color: value ? style.color?.withOpacity(0.5) : style.color) ??
  ...
  Widget withGradient(LinearGradient gradient) {
    return ShaderMask(shaderCallback: (Rect bounds) => gradient.createShader(bounds), child: this);
  }
}
